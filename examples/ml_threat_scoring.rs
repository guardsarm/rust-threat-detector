...
use rust_threat_detector::{
    MLThreatScorer, ThreatFeatures, RiskLevel,
};
...
fn main() {
    ...
    for factor in &score.contributing_factors {
        println!("  - {}: {:.3} ({})", factor.name, factor.contribution, factor.description);
    }
    ...
    println!("\n--- Example 3: Batch Scoring ---");
    let batch_features: Vec<ThreatFeatures> = (0..5).map(|i| {
    ...
    for (i, score) in batch_scores.iter().enumerate() {
        println!("Entity {}: {:?} (score: {:.2})", i, score.risk_level, score.score);
    }
    ...
    for threat in top_threats {
        println!("  - Score: {:.2}, Level: {:?}", threat.score, threat.risk_level);
    }
    ...
    println!("Risk Assessment: {}", match analysis_score.risk_level {
